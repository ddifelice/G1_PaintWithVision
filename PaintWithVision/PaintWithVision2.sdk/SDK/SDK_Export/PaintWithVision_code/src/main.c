...
int main()
{
	...
	FillBuffer(DrawBuffer,0x00000000); //black
	...
	// initial search box
	int search_box_radius = 240;
	int search_box_x = DISPLAY_COLUMNS/2;
	int search_box_y = DISPLAY_ROWS/2;
	...
	//alanwu: change search box on the go
    //OnePass(&blobList, (char*) FrameBuffer, 640/2, 480/2, 100);
	if (miss_counter > 3)
	{
	...
	{
		OnePass(&blobList, (char*) FrameBuffer, search_box_x, search_box_y, search_box_radius);
	}
	//DrawSqure(search_box_x , search_box_y, search_box_radius, 2, DrawBuffer, 15,0,0 );

	// draw the found blobs
	int i, avg_x, avg_y, radius = 0;
	struct Blob blob;
	/*for (i = 0; i < blobList.size; i ++){
		...
		//FillColor(avg_x-radius, avg_y-radius, avg_x+radius, avg_y+radius, (char*) DrawBuffer, avg_b,avg_g,avg_r);

		//FillColor(x-2, y-2, x+2, y+2, (char*) DrawBuffer, 15,15,15);

	}*/

			//alanwu: showing the biggest blob
			int current_max_count = 100;
			struct Blob finalBlob;
			finalBlob.count = 0;
			for (i = 0; i < blobList.size; i ++)
			{
				blob = vector_get(&blobList, i);
				if (blob.count > current_max_count)
				{
					finalBlob = blob;
				}
			}

			if (finalBlob.count > 0)
			{
				miss_counter = 0;
				avg_x = finalBlob.total_x/finalBlob.count;
				avg_y = finalBlob.total_y/finalBlob.count;
				radius = sqrt(finalBlob.count)/2;

				search_box_x = avg_x;
				search_box_y = avg_y;
				search_box_radius = radius * 2;
				FillColor(avg_x-radius, avg_y-radius, avg_x+radius, avg_y+radius, (char*) DrawBuffer, 0, 5, 15);
			}
			else
			{
				miss_counter++;
