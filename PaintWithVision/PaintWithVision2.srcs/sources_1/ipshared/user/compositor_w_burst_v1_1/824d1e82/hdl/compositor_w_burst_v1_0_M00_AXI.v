...
//	assign M_AXI_AWADDR	= C_M_TARGET_SLAVE_BASE_ADDR + axi_awaddr; NOt sure why it is add?
...
//	assign M_AXI_ARADDR	= C_M_TARGET_SLAVE_BASE_ADDR + axi_araddr; Not Sure why it is add
...
//	    if (M_AXI_ARESETN == 0 || init_txn_pulse == 1'b1 || start_single_burst_write == 1'b1)
	    if (M_AXI_ARESETN == 0| start_single_burst_write == 1'b1 | Start[1] )     
	      begin                                                                         
	        write_index <= 0;                                                           
	      end                                                                           
...
//	    if (M_AXI_ARESETN == 0 || init_txn_pulse == 1'b1 || start_single_burst_read)
	    if (M_AXI_ARESETN == 0 | start_single_burst_read | Start[1] )                  
	      begin                                                             
	        read_index <= 0;                                                
	      end                                                               
...
//		if (M_AXI_ARESETN == 0 || init_txn_pulse == 1'b1)// || M_AXI_RLAST)             
		if (M_AXI_ARESETN == 0 | Start[1] )// || M_AXI_RLAST)
			expected_rdata <= 'b1;                            
...
//	    if (M_AXI_ARESETN == 0 || init_txn_pulse == 1'b1)
	    ...
	    else if (M_AXI_BVALID && (write_index == C_M_AXI_BURST_LEN-1) && axi_bready) //&& (write_burst_counter[C_NO_BURSTS_REQ]) )                          
	      writes_done <= 1'b1;                                                                                  
	    else                                                                                                    
...
AddressGenerator # (
    .C_M_AXI_BURST_LEN(C_M_AXI_BURST_LEN)
            ...
            .DrawBufferAddrBase(DrBAddrBase),
            .DispBufferAddrBase(DispBAddrBase),
            .clk(M_AXI_ACLK),
            .rst(M_AXI_ARESETN & ~Start[1] ),
            .next(next),
            .done(done_ag)  
            );
    ...
    //        if (M_AXI_ARESETN == 0 || init_txn_pulse == 1'b1)                                          
        ...
        begin
            ...
            begin 
                   for (i = 0; i < C_M_AXI_BURST_LEN; i = i + 1)               
                   begin
                        if (i == read_index & M_AXI_RDATA[31])
                        begin
                            DrawBuffer[i] <= M_AXI_RDATA; 
                        end
                        else
    ...
    always @(posedge M_AXI_ACLK)                                       
        ...
        //        if (M_AXI_ARESETN == 0 || init_txn_pulse == 1'b1)                                          
            ...
            if (M_AXI_WREADY & axi_wvalid) //&& (write_index != C_M_AXI_BURST_LEN-1))
            begin                 
	...
	// User logic ends
    assign x_pos = mst_exec_state;
    assign y_pos = DrawBuffer[0];
